...
2. **API·옵션 확인** — 부족할 때만 **real-mcp MCP** (`product: realgrid` 또는 `realchart`)의 `search_resources` → 필요 시 `get_resource`. 웹 검색·기억·`node_modules` 번들·`.d.ts` 분석에 의존하지 않는다.
...
1. **`search_resources`** — RAG 검색. 스니펫만으로 구현 가능하면 **그대로 사용**하고 구현으로 넘어간다. 부족할 때만 `get_resource`.
